...
type CheckOwnerResult = { ok: true; userId: string; slug: string } | {
  ok: false;
  error: string;
};
...
export async function checkOwner(
  ...
  const storyOwner = await sql.query(
    "SELECT user_id, slug FROM stories WHERE story_id = $1 AND (user_id = $2 OR $3 = 'admin')",
  ...
  if (storyOwner.length === 1) {
    return {
      ok: true,
      userId: storyOwner[0].user_id,
      slug: storyOwner[0].slug,
    };
  }
